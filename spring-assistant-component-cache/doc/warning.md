...
例如：
        ...
        public CollectionDTO<AccountDTO> getAccountByCriteria(AccountCriteriaDTO criteria)  {
            List<AccountDTO> list=null;
            PaginationDTO paging=criteria.getPaging();
            if(null != paging && paging.getPageNo() > 0 && paging.getPageSize() > 0) {// 如果需要分页查询，先查询总数
                ...
                return new CollectionDTO<AccountDTO>(list, recordCnt, criteria.getPaging().getPageSize());
            } else {
                list=accountDAO.getAccountByCriteria(criteria);
                return new CollectionDTO<AccountDTO>(list, null != list ? list.size() : 0, 0);
            }
        }
...
例如：
        ...
        public List<AccountDTO> getDistinctAccountByPlayerGet(AccountCriteriaDTO criteria) {
