...
public class AutoLoadHandler {
    ...
    public AutoLoadHandler(CacheHandler cacheHandler, AutoLoadConfig config) {
        ...
        if (this.config.getThreadCnt() > 0) {
            ...
            this.autoLoadMap =
                    new ConcurrentHashMap<CacheKeyDTO, AutoLoadDTO>(this.config.getMaxElement());
            this.autoLoadQueue = new LinkedBlockingQueue<AutoLoadDTO>(this.config.getMaxElement());
    ...
    public AutoLoadDTO getAutoLoadTO(CacheKeyDTO cacheKey) {
        if (null == autoLoadMap) {
            return null;
        }
    ...
    public void removeAutoLoadTO(CacheKeyDTO cacheKey) {
        if (null == autoLoadMap) {
            return;
        }
     ...
     */
    public void resetAutoLoadLastLoadTime(CacheKeyDTO cacheKey) {
        if (null == autoLoadMap) {
            return;
        }
        AutoLoadDTO autoLoadTO = autoLoadMap.get(cacheKey);
    ...
    public AutoLoadDTO putIfAbsent(
        ...
        if (expire >= AUTO_LOAD_MIN_EXPIRE && autoLoadMap.size() <= this.config.getMaxElement()) {
            ...
            autoLoadTO = new AutoLoadDTO(cacheKey, joinPoint, arguments, cache, expire);
            AutoLoadDTO tmp = autoLoadMap.putIfAbsent(cacheKey, autoLoadTO);
            if (null == tmp) {
                return autoLoadTO;
            } else {
                return tmp;
    ...
    public AutoLoadDTO[] getAutoLoadQueue() {
        ...
        AutoLoadDTO[] tmpArr = new AutoLoadDTO[autoLoadMap.size()];
        // 复制引用
    ...
    class SortRunnable implements Runnable {
        ...
        public void run() {
            while (running) {
                ...
                AutoLoadDTO[] tmpArr = getAutoLoadQueue();
                if (null == tmpArr || tmpArr.length == 0) {
                    continue;
                }
                for (int i = 0; i < tmpArr.length; i++) {
                    try {
                        AutoLoadDTO to = tmpArr[i];
    ...
    class AutoLoadRunnable implements Runnable {
        ...
        public void run() {
            while (running) {
                try {
                    AutoLoadDTO tmpTO = autoLoadQueue.take();
                    if (null != tmpTO) {
                        loadCache(tmpTO);
        ...
        private void loadCache(AutoLoadDTO autoLoadTO) {
            if (null == autoLoadTO) {
                return;
            }
            ...
            if (requestTimeout > 0
                    && (now - autoLoadTO.getLastRequestTime())
                            >= requestTimeout * ONE_THOUSAND_MS) {
            ...
            if (difFirstRequestTime > oneHourSecs
                    && autoLoadTO.getAverageUseTime() < ONE_THOUSAND_MS
            ...
            if (config.isCheckFromCacheBeforeLoad()) {
                try {
                    ...
                    result =
                            cacheHandler.get(
                                    autoLoadTO.getCacheKey(), method, autoLoadTO.getArgs());
                } catch (Exception ex) {
            ...
            CacheKeyDTO cacheKey = autoLoadTO.getCacheKey();
            ...
            try {
                newCacheWrapper =
                        dataLoader
                                .init(pjp, autoLoadTO, cacheKey, cache, cacheHandler)
                                .loadData()
                                .getCacheWrapper();
            } catch (Throwable e) {
                logger.error(e.getMessage(), e);
                ...
                try {
                    if (null != newCacheWrapper) {
                        cacheHandler.writeCache(
                                pjp, autoLoadTO.getArgs(), cache, cacheKey, newCacheWrapper);
                        autoLoadTO
