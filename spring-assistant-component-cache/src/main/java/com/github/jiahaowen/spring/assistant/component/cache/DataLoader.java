...
import com.github.jiahaowen.spring.assistant.component.cache.exception.LoadDataTimeOutException;
...
public class DataLoader {
    ...
    private CacheKeyDTO cacheKey;

    private Cache cache;

    private Object[] arguments;

    private AutoLoadDTO autoLoadTO;
    ...
    public DataLoader init(
            CacheAopProxyChain pjp, CacheKeyDTO cacheKey, Cache cache, CacheHandler cacheHandler) {
    ...
    public DataLoader loadData() throws Throwable {
        ProcessingDTO processing = cacheHandler.processing.get(cacheKey);
        if (null == processing) {
            ProcessingDTO newProcessing = new ProcessingDTO();
            ProcessingDTO firstProcessing =
                    cacheHandler.processing.putIfAbsent(
                            cacheKey, newProcessing); // 为发减少数据层的并发，增加等待机制。
        ...
        if (isFirst) {
            logger.trace("{} first thread!", threadName);
            try {
    ...
    private void doFirstRequest(ProcessingDTO processingTO) throws Throwable {
    ...
    private void doWaitRequest(ProcessingDTO processing, Object lock) throws Throwable {
        ...
        do { // 等待
            ...
            if (processing.isFirstFinished()) {
                CacheWrapper<Object> tmpcacheWrapper =
                        processing.getCache(); // 从本地内存获取数据， 防止频繁去缓存服务器取数据，造成缓存服务器压力过大
                logger.trace(
                        "{} do FirstFinished" + " is null :{}", tname, (null == tmpcacheWrapper));
                ...
                if (null != error) { // 当DAO出错时，直接抛异常
                    logger.trace("{} do error", tname);
                    throw error;
                }
                break;
            } else {
                synchronized (lock) {
                    logger.trace("{} do wait", tname);
                    try {
