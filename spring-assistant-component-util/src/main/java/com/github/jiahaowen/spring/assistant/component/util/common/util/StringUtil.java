...
public class StringUtil {
     ...
     *
     * <p>此方法实际上和<code>defaultIfNull(String)</code>等效。
     *
     * <pre>
    ...
    private static final WordTokenizer UPPER_CASE_WITH_UNDERSCORES_TOKENIZER =
            new WordTokenizer() {
                @Override
                protected void startSentence(StringBuffer buffer, char ch) {
                    buffer.append(Character.toUpperCase(ch));
    ...
    private static final WordTokenizer LOWER_CASE_WITH_UNDERSCORES_TOKENIZER =
            new WordTokenizer() {
                @Override
                protected void startSentence(StringBuffer buffer, char ch) {
                    buffer.append(Character.toLowerCase(ch));
     ...
     * 将字符串按空白字符分割。
     *
     * <p>分隔符不会出现在目标数组中，连续的分隔符就被看作一个。如果字符串为<code>null</code>，则返回<code>null</code>。
     *
     * <pre>
    ...
    /**
     * 在字符串中查找指定字符集合中的字符，并返回第一个匹配的起始索引。 如果字符串为<code>null</code>，则返回<code>-1</code>。 如果字符集合为<code>
     * null</code>或空，也返回<code>-1</code>。
     *
     * <pre>
    ...
    /**
     * 在字符串中查找指定字符串集合中的字符串，并返回第一个匹配的起始索引。 如果字符串为<code>null</code>，则返回<code>-1</code>。 如果字符串集合为<code>
     * null</code>或空，也返回<code>-1</code>。 如果字符串集合包括<code>""</code>，并且字符串不为<code>null</code>，则返回<code>
     * str.length()</code>
     *
     * <pre>
     * StringUtil.indexOfAny(null, *)                     = -1
    ...
    /**
     * 在字符串中查找不在指定字符集合中的字符，并返回第一个匹配的起始索引。 如果字符串为<code>null</code>，则返回<code>-1</code>。 如果字符集合为<code>
     * null</code>或空，也返回<code>-1</code>。
     *
     * <pre>
    ...
    /**
     * 从字符串尾部开始查找指定字符串集合中的字符串，并返回第一个匹配的起始索引。 如果字符串为<code>null</code>，则返回<code>-1</code>。 如果字符串集合为
     * <code>null</code>或空，也返回<code>-1</code>。 如果字符串集合包括<code>""</code>，并且字符串不为<code>null</code>，则返回
     ...
     *
     * <p>如果字符串为<code>null</code>，则返回<code>false</code>。 如果字符集合为<code>null</code>则返回<code>false
     * </code>。 但是空字符串永远返回<code>true</code>.
     *
     * <pre>
     ...
     *
     * <p>如果字符串为<code>null</code>，则返回<code>false</code>。 如果字符集合为<code>null</code>则返回<code>true
     * </code>。 但是空字符串永远返回<code>true</code>.
     *
     * <pre>
     ...
     * 取指定字符串的子串。
     *
     * <p>负的索引代表从尾部开始计算。如果字符串为<code>null</code>，则返回<code>null</code>。
     *
     * <pre>
     ...
     * 取得第一个出现的分隔子串之前的子串。
     *
     * <p>如果字符串为<code>null</code>，则返回<code>null</code>。 如果分隔子串为<code>null</code>或未找到该子串，则返回原字符串。
     *
     * <pre>
     ...
     * 取得指定分隔符的前两次出现之间的子串。
     *
     * <p>如果字符串为<code>null</code>，则返回<code>null</code>。 如果分隔子串为<code>null</code>，则返回<code>null
     * </code>。
     *
     * <pre>
     ...
     * 替换指定的子串，只替换第一个出现的子串。
     *
     * <p>如果字符串为<code>null</code>则返回<code>null</code>，如果指定子串为<code>null</code>，则返回原字符串。
     *
     * <pre>
     ...
     * 将字符串中所有指定的字符，替换成另一个。
     *
     * <p>如果字符串为<code>null</code>则返回<code>null</code>。
     *
     * <pre>
     ...
     * 将字符串中所有指定的字符，替换成另一个。
     *
     * <p>如果字符串为<code>null</code>则返回<code>null</code>。如果搜索字符串为<code>null</code>或空，则返回原字符串。
     *
     * <p>例如： <code>replaceChars(&quot;hello&quot;, &quot;ho&quot;, &quot;jy&quot;) = jelly</code>。
     *
     * <p>通常搜索字符串和替换字符串是等长的，如果搜索字符串比替换字符串长，则多余的字符将被删除。 如果搜索字符串比替换字符串短，则缺少的字符将被忽略。
     *
     * <pre>
     * StringUtil.replaceChars(null, *, *)           = null
     * StringUtil.replaceChars("", *, *)             = ""
     ...
     * 将指定的子串用另一指定子串覆盖。
     *
     * <p>如果字符串为<code>null</code>，则返回<code>null</code>。 负的索引值将被看作<code>0</code>
     ...
     *
     * <ul>
     *   <li>如果<code>str</code>比<code>maxWidth</code>短，直接返回；
     *   <li>否则将它转换成缩略：<code>substring(str, 0, max-3) + "..."</code>；
     *   <li>如果<code>maxWidth</code>小于<code>4</code>抛出<code>IllegalArgumentException</code>；
     *   <li>返回的字符串不可能长于指定的<code>maxWidth</code>。
     * </ul>
     *
     * <pre>
     ...
     *
     * <p>和<code>abbreviate(String, int)</code>类似，但是增加了一个“左边界”偏移量。
     * 注意，“左边界”处的字符未必出现在结果字符串的最左边，但一定出现在结果字符串中。
     *
     * <p>返回的字符串不可能长于指定的<code>maxWidth</code>。
     *
     * <pre>
     * StringUtil.abbreviate(null, *, *)                = null
     ...
     *
     * <p>这个算法取自<a href="http://www.merriampark.com/ld.htm">http://www.merriampark.com/ld.htm</a>。
