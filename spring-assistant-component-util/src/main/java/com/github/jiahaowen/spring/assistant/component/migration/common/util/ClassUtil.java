...
public class ClassUtil {
     ...
     * 取得对象所属的类的直观类名。
     *
     * <p>相当于 <code>object.getClass().getName()</code> ，但不同的是，该方法用更直观的方式显示数组类型。 例如：
     *
     * <pre>
     ...
     *  </pre>
     *
     * <p>对于非数组的类型，该方法等效于 <code>Class.getName()</code> 方法。
     *
     * <p>注意，该方法所返回的数组类名只能用于显示给人看，不能用于 <code>Class.forName</code> 操作。
     *
     ...
     * 取得直观的类名。
     *
     * <p>相当于 <code>clazz.getName()</code> ，但不同的是，该方法用更直观的方式显示数组类型。 例如：
     *
     * <pre>
     *  int[].class.getName() = "[I" ClassUtil.getClassName(int[].class) = "int[]"
     *
     ...
     * 取得直观的类名。
     *
     * <p><code>className</code> 必须是从 <code>clazz.getName()</code> 所返回的合法类名。该方法用更直观的方式显示数组类型。 例如：
     *
     * <pre>
     *  int[].class.getName() = "[I" ClassUtil.getClassName(int[].class) = "int[]"
     *
    ...
    public static String getPackageNameForObjectAsResource(Object object) {
        ...
        return getPackageNameForObject(object)
     ...
     * 取得指定类的所有父类。
     *
     * <p>对于一个 <code>Class</code> 实例，如果它不是接口，也不是数组，此方法依次列出从该类的父类开始直到 <code>Object</code> 的所有类。
     *
     * <p>例如 <code>ClassUtil.getSuperclasses(java.util.ArrayList.class)</code> 返回以下列表：
     *
     * <ol>
     *   <li><code>java.util.AbstractList</code>
     *   <li><code>java.util.AbstractCollection</code>
     *   <li><code>java.lang.Object</code>
     * </ol>
     *
     * <p>对于一个接口，此方法返回一个空列表。
     *
     * <p>例如<code>ClassUtil.getSuperclasses(java.util.List.class)</code>将返回一个空列表。
     *
     * <p>对于一个数组，此方法返回一个列表，列出所有component类型的父类的相同维数的数组类型。 例如： <code>
     * ClassUtil.getSuperclasses(java.util.ArrayList[][].class)</code> 返回以下列表：
     *
     * <ol>
     *   <li><code>java.util.AbstractList[][]</code>
     *   <li><code>java.util.AbstractCollection[][]</code>
     ...
     * <p>对于一个 <code>Class</code> 实例，如果它不是接口，也不是数组，此方法依次列出从该类的父类开始直到 <code>Object</code> 的所有类。
     *
     * <p>例如 <code>ClassUtil.getInterfaces(java.util.ArrayList.class)</code> 返回以下列表：
     *
     * <ol>
     *   <li><code>java.util.List</code>
     *   <li><code>java.util.Collection</code>
     *   <li><code>java.util.RandomAccess</code>
     *   <li><code>java.lang.Cloneable</code>
     *   <li><code>java.io.Serializable</code>
     * </ol>
     *
     * <p>对于一个数组，此方法返回一个列表，列出所有component类型的接口的相同维数的数组类型。 例如： <code>
     * ClassUtil.getInterfaces(java.util.ArrayList[][].class)</code> 返回以下列表：
     *
     * <ol>
     *   <li><code>java.util.List[][]</code>
     *   <li><code>java.util.Collection[][]</code>
     ...
     * </ol>
     *
     * <p>注意，原子类型及其数组，将被转换成相应的包装类来处理。 例如： <code>ClassUtil.getInterfaces(int[][].class)</code>
     * 返回以下列表：
     *
     * <ol>
     *   <li><code>java.lang.Comparable[][]</code>
     *   <li><code>java.io.Serializable[][]</code>
     * </ol>
     *
     * @param clazz 要查找的类
     ...
     *
     * <p>此方法可以用来确定指定类型的参数 <code>object1, object2, ...</code> 是否可以用来调用确定参数类型为 <code>class1, class2,
     * ...</code> 的方法。
     *
     * <p>对于 <code>fromClasses</code> 的每个元素 <code>fromClass</code> 和 <code>classes</code> 的每个元素
     ...
     *   <li>调用 <code>Class.isAssignableFrom</code> 方法来确定目标类 <code>clazz</code> 是否和参数类 <code>
     *       fromClass</code> 相同或是其父类、接口，如果是，则返回 <code>true</code>。
     ...
     *       Conversion规则，参数类型 <code>fromClass</code> 可以是任何能扩展成该目标类型的原子类型及其包装类。 例如， <code>clazz
     *       </code> 为 <code>long</code> ，那么参数类型可以是 <code>byte</code>、 <code>short</code>、<code>int
     *       </code>、<code>long</code>、<code>char</code> 及其包装类 <code>java.lang.Byte</code>、<code>
     *       java.lang.Short</code>、<code>java.lang.Integer</code>、 <code>java.lang.Long</code> 和
     *       <code>java.lang.Character</code> 。如果满足这个条件，则返回 <code>true</code>。
     *   <li>不满足上述所有条件，则返回 <code>false</code>。
     * </ol>
     *
     ...
     *
     * <p>此方法可以用来确定指定类型的参数 <code>object1, object2, ...</code> 是否可以用来调用确定参数类型 <code>class1, class2,
     * ...</code> 的方法。
     *
     * <p>按照如下规则：
     *
